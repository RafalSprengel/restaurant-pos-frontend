  ...
  const SingleCategory = ({ category }) => {
    return (
      <div className="menu__item-categories__list__item" onClick={() => handleCategoryClick(category.name)}>
        <div className={'menu__item-categories__list__item__icon-container ' + (category.name == currentCategory ? 'menu__item-categories__list__item__icon-container--active' : '')}>
          <span className="material-symbols-outlined">
            {/* {el.googleIconName} */}
            iC
          </span>
        </div>
        <div className={'menu__item-categories__list__item__name-container ' + (category.name == currentCategory ? 'menu__item-categories__list__item__name-container--active' : '')}>{category.name}</div>
      </div>
    );
  };
