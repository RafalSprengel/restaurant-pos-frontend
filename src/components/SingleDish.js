...
export const SingleDish = ({ singleDish }) => {
     ...
     return (
          <div className="single-dish" role="button" tabIndex={0}>
               {Boolean(getItemQuantity(singleDish._id) > 0) && <div className="single-dish__quantity">{getItemQuantity(singleDish._id)}</div>}
               <img className="single-dish__img" alt={singleDish.name || 'Dish image'} src={process.env.PUBLIC_URL + singleDish.img} />
