...
const CategoriesList = () => {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            else setErrorDeleteCategory(response.data?.error || 'Failed to delete the category.');
        } catch (error) {
            setErrorDeleteCategory(error.response?.data?.error || 'Failed to delete the category.');
        } finally {
            setIsDeleting(false);
    ...
    return (
        <div className="categories-list__container">
            ...
            {errorDeleteCategory && <ErrorMessage message={errorDeleteCategory} />}
            {fetchError && <ErrorMessage message={`Error fetching categories: ${fetchError.toString()}`} />} 
                                    ...
                                    <tr key={item._id} onClick={() => handleRowClick(item._id)} style={{ cursor: 'pointer' }}>
                                        ...
                                        <td className="categories-list__table-cell--actions">
                                            <button
                                                ...
                                                disabled={isDeleting} 
                                            >
                                                {isDeleting && categoryToDelete === item._id ? <Loader size={16} color="currentColor" /> : <IconTrash size={16} />}
                                            </button>
                                        </td>
                                    </tr>
                                ))}
            ...
            {showModal && (
                <ConfirmationModal
                    ...
                    message={`Are you sure you want to delete this category? ${isDeleting ? ' (Deleting...)' : ''}`}
                />
            )}
        </div>
    );
