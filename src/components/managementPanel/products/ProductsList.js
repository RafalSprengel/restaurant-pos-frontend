...
const ProductsList = () => {
    ...
    const getProducts = async () => {
        ...
        try {
            ...
            setErrorFetchProducts(error.response?.data?.error || error.message || "Error fetching products");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const rows = productsList.map((item) => (
        <tr key={item._id} onClick={() => handleRowClick(item._id)} style={{ cursor: 'pointer' }}>
            ...
            <td className="products-list__table-cell--actions">
                <button
                    className="products-list__delete-button"
                    onClick={(e) => handleDeleteClick(e, item._id)}
                    disabled={isDeleting && productToDelete === item._id}
                >
                    {isDeleting && productToDelete === item._id ? <Loader size={16} color="currentColor" /> : <IconTrash size={16} />}
                </button>
            </td>
        </tr>
    ));
        ...
        <div className="products-list__container">
                        ...
                        {productsList.map((product) => (
                            <div key={product._id}
                                ...
                                <div className="products-list__card-buttons-group">
                                    <button className="products-list__card-button-edit" onClick={(e) => { e.stopPropagation(); handleRowClick(product._id); }}>
                                        <IconEdit size={16} /> Edit
                                    </button>
                                    <button
                                        className="products-list__card-button-delete"
                                        onClick={(e) => handleDeleteClick(e, product._id)}
                                        disabled={isDeleting && productToDelete === product._id}
                                    >
                                        {isDeleting && productToDelete === product._id ? <Loader size={16} color="currentColor" /> : <IconTrash size={16} />} Delete
                                    </button>
                                </div>
                            </div>
                        ))}
            ...
            {showModal && (
                <ConfirmationModal
                    ...
                    message={`Are you sure you want to delete this product? ${isDeleting ? ' (Deleting...)' : ''}`}
                />
            )}
        </div>
    );
